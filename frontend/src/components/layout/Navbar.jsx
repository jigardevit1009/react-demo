    ...
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          type="button"
          onClick={toggleTheme}
          title={`Switch to ${isDark ? "Light" : "Dark"} Mode (Context API)`}
        ...
        >
          <span>{isDark ? "☀️" : "🌙"}</span>
          <span className="text-xs hidden sm:inline capitalize font-semibold">{theme}</span>
        </button>

        {/* User Info */}
        <div className="flex items-center gap-3 pl-3 border-l border-gray-200 dark:border-gray-800">
          <div className="text-right hidden sm:block">
            <p className={`text-sm font-semibold leading-none ${isDark ? "text-gray-100" : "text-gray-800"}`}>
              {displayName}
            </p>
          ...
          {isAuthenticated && (
            <span className="h-2 w-2 rounded-full bg-emerald-500 ring-2 ring-white dark:ring-gray-900" title="Online" />
          )}
        </div>
    </div>
    </header>
